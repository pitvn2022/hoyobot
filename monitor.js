...
function autoUpdate() {
  ...
  exec('git pull', (err, stdout) => {
    ...
    if (/Already up to date\./.test(stdout)) {
      ...
      notifyAll('🚀 New code pulled, restarting...', 'UP').catch(e => logError('Monitor', e));
      if (botProcess && isUp) {
...
// --- Log tailing with optional filter ---
function tailLogs(filter) {
  try {
...
app.get('/', (req, res) => {
  ...
  const logs    = tailLogs(filter).map(l => l.replace(/</g,'&lt;').replace(/>/g,'&gt;')).join('\n');
...
<form method="GET">
  <input name="filter" placeholder="Filter logs" value="${filter}"/>
  <button type="submit">Search</button>
  ${filter ? `<button onclick="window.location='/'">Clear</button>` : ''}
</form>
<p>Status: <span class="status">${status}</span> (${formatDuration(sinceMs)})</p>
...
<h2>📝 Recent Logs (last ${MAX_LOG_LINES} lines${filter?` filtered by "${filter}"`:''})</h2>
<pre>${logs}</pre>
</body></html>`);
});
